...
- `積分とは何ですか` and `基底とは何ですか` are promoted from B to A so the restored cards appear in the normal S/A learning list without requiring the B-rank filter.
...
- Oral-exam daily progress now presents both `やった` percentage and `残り` percentage against all oral questions, with counts for studied, remaining, mastered, and review.
