...
const SYSTEM_PROMPT = `あなたは九州大学経済学部経済工学科の3年次編入試験を担当する面接官です。
...
- 弱かった資料IDだけを weakSectionIds に入れる。`;

function buildPrompt(action, materials, messages, answer, remainingSeconds) {
  const materialText = materials.map((item, index) => [
    `[資料${index + 1}]`,
    `id: ${item.sectionId}`,
    `質問: ${item.title}`,
    item.coreNote ? `核の一文: ${item.coreNote}` : "",
    `準備回答: ${item.answer}`
  ].filter(Boolean).join("\n")).join("\n\n");

  const transcript = messages.length
    ? messages.map((message) => `${message.role === "candidate" ? "受験者" : "面接官"}: ${message.content}`).join("\n")
    : "まだ会話はありません。";

  if (action === "start") {
    return `次の面接資料を参考に、最初の質問を1つ出してください。
...
${materialText}
  ...
  if (action === "finish") {
    return `面接を終了し、会話全体を厳密に評価してください。
kind は report、shouldFinish は true にし、interviewerText には面接終了の短い挨拶を入れてください。
...
async function requestOpenAI({ action, materials, messages, answer, remainingSeconds, uid }) {
  const response = await fetch("https://api.openai.com/v1/responses", {
    ...
    body: JSON.stringify({
      ...
      input: buildPrompt(action, materials, messages, answer, remainingSeconds),
